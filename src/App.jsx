  ...
  useEffect(() => {
    async function bootstrap() {
      try {
        ...
        setBootError(error.message || 'Failed to connect to backend/database.')
      } finally {
        setLoading(false)
      }
    }
  ...
  if (bootError) {
    return (
      <div className="flex min-h-screen items-center justify-center p-6">
        <div className="max-w-xl rounded-xl border border-rose-200 bg-rose-50 p-4 text-rose-700">
          <p className="font-semibold">Could not load app data.</p>
          <p className="mt-1 text-sm">{bootError}</p>
          <p className="mt-2 text-sm">Check backend logs using: <code>docker-compose logs backend</code></p>
        </div>
      </div>
    )
  }
